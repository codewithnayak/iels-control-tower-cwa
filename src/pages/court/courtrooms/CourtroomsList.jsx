...
export default function CourtroomsList() {
  const [filter, setFilter] = useState("");
  const [page, setPage] = useState(0);
  const pageSize = 5;

  const filtered = courtroomsData.filter((c) =>
  ...
  return (
    <MainLayout>
      <h1 className="text-3xl font-bold mb-8">Courtrooms</h1>

      {/* Search + Add */}
      <div className="flex justify-between items-center mb-6">
        <input
          className="border p-2 rounded w-1/3 shadow-sm"
          placeholder="Search courtrooms..."
          value={filter}
